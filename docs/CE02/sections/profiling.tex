...
\subsection{Temporización estadística}

La primera técnica consiste en ejecutar el binario \texttt{mul\_seq\_perf}
un total de $\text{TIMING\_RUNS} = 10$ veces de forma independiente y registrar
el tiempo reportado por el propio programa en milisegundos.
La primera ejecución se descarta como \textit{warm-up}: su propósito es poblar
la caché de instrucciones y datos del procesador para que las corridas medidas
no incluyan el costo de la carga inicial desde memoria principal.

Con los valores obtenidos se calcula la media aritmética $\bar{t}$ y la desviación
estándar $\sigma_t$.
El coeficiente de variación
\begin{equation}
    CV = \frac{\sigma_t}{\bar{t}} \times 100\%
\end{equation}
cuantifica la estabilidad experimental: un $CV$ bajo (generalmente menor al 2\%)
indica que las mediciones son reproducibles y no están contaminadas por
interferencia del sistema operativo o por efectos térmicos del procesador.
Un $CV$ elevado, en cambio, sugiere que el sistema no se encontraba en condiciones
estables durante la medición.

El rendimiento en GFLOP/s se deriva de la temporización mediante la relación
\begin{equation}
    \text{GFLOPS} = \frac{2N^3}{\bar{t} \times 10^{-3} \times 10^{9}},
\end{equation}
donde $2N^3$ es el número exacto de operaciones de multiplicación-acumulación
del algoritmo estándar de multiplicación de matrices densas.
Este valor normaliza el rendimiento respecto al tamaño del problema y permite
comparar directamente distintas implementaciones y arquitecturas.
...
en tiempo de compilación~\cite{graham1982gprof}.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{-fno-inline}}: impide que el compilador reemplace el cuerpo
        ...
        Sin esta flag, bajo optimización \texttt{-O2}, funciones pequeñas del kernel
        de multiplicación podrían desaparecer del perfil porque ya no existen como
        entidades separadas en el código objeto.
\end{itemize}
...
La limitación inherente de \texttt{gprof} es que su medición del tiempo se basa
...
El script lo ejecuta con la flag \texttt{-r~5}, lo que indica a \texttt{perf}
que repita la ejecución cinco veces y reporte el promedio de cada contador junto
con su coeficiente de variación, brindando así información estadística sobre la
estabilidad de las mediciones de hardware.
...
el algoritmo: el orden canónico $i$-$j$-$k$ produce accesos no contiguos a la
...
reside en cuantificar la \textbf{distribución efectiva de latencias}: para
...
\begin{itemize}
    ...
    \item \textbf{Extra heap bytes}: overhead interno del gestor de memoria
        (\textit{metadata} de los bloques, alineación y fragmentación interna).
\end{itemize}
...
a nivel de instrucción~\cite{nethercote2007valgrind}.
...
Las seis técnicas de perfilado del script cubren dimensiones ortogonales del
...
La Tabla~\ref{tab:seq_profiling} consolida los resultados de las seis técnicas de perfilado aplicadas a la multiplicación de matrices clásica (bucle canónico $i$-$j$-$k$) para tamaños crecientes, $N \in \{128, 256, 512, 1024\}$. La triangulación de herramientas permite identificar con precisión el origen de las limitaciones de rendimiento observadas.
...
Las herramientas de perfilado coinciden en señalar la función de multiplicación como la única fuente relevante de cómputo. El reporte de gprof le atribuye el 100\% del tiempo de ejecución, mientras que perf record indica que para $N=1024$ esta función concentra el 98.45\% de la actividad en el procesador, dejando apenas un 1.5\% restante a rutinas del sistema como la inicialización y el llenado de matrices. Valgrind Cachegrind refuerza este resultado al asociarle el 98.5\% de las referencias de instrucción contabilizadas. El diagnóstico es directo: todo el peso computacional recae sobre los tres bucles anidados del algoritmo, y cualquier optimización debe dirigirse exclusivamente a esa región.
...
Las métricas recopiladas con perf stat muestran una caída sostenida del rendimiento a medida que crece el tamaño de la matriz, pasando de 5.59 GFLOPS para $N=128$ hasta 3.05 GFLOPS para $N=1024$.
